defmodule Squares do
  ...
  def sum_of_squares(number), do: 1..number |> Enum.map(&:math.pow(&1,2)) |> Enum.sum
  
  @doc """
  Calculate square of sums from 1 to a given end number.
  """
  @spec square_of_sums(pos_integer) :: pos_integer
  def square_of_sums(number), do: 1..number |> Enum.sum |> :math.pow(2)
  ...
  def difference(number), do: square_of_sums(number) - sum_of_squares(number)
