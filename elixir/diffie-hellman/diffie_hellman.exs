defmodule DiffieHellman do
  ...
  def generate_private_key(prime_p), do: 12 |> :crypto.strong_rand_bytes |> to_seed_tuple |> generate_private_key(prime_p)

  def generate_private_key(seed, prime_p), do: with _state <- :exsplus |>:rand.seed(seed), do: 1..(prime_p - 1) |> Enum.random

  def to_seed_tuple(<< i1 :: unsigned-integer-32, i2 :: unsigned-integer-32, i3 :: unsigned-integer-32>>), do: {i1, i2, i3}  
  ...
  def generate_public_key(prime_p, prime_g, private_key), do: prime_g |> :crypto.mod_pow(private_key, prime_p) |> :binary.decode_unsigned 
  ...
  def generate_shared_secret(prime_p, public_key_b, private_key_a), do: prime_p |> generate_public_key(public_key_b, private_key_a)
